...
func (dm *domainMap) getWithEtcdClient(store kv.Storage, etcdClient *clientv3.Client) (d *domain.Domain, err error) {
	...
	err = util.RunWithRetry(util.DefaultMaxRetries, util.RetryInterval, func() (retry bool, err1 error) {
		...
		sysksSessFactory := getSysKSSessionFactory(store)
		sysFactory := getSessionFactoryWithDom(store)
		d = domain.NewDomainWithEtcdClient(store, ddlLease, statisticLease, planReplayerGCLease, factory, sysksSessFactory, etcdClient)
