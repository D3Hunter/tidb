...
func TestDoImport(t *testing.T) {
	...
	t.Run("dispatcher waits for error shutdown when result channel closes", func(t *testing.T) {
		...
		select {
		case err := <-errCh:
			require.FailNow(t, "dispatcher returned before shutdown outcome", "error: %v", err)
		...
		case err := <-errCh:
			require.ErrorIs(t, err, context.Canceled)
		case <-time.After(10 * time.Second):
			t.Fatal("timed out waiting for dispatcher to return")
		}
	})
}
